...
export interface TaskProgress {completed?:number;total?:number;phase?:string;elapsed_seconds?:number}
export interface ModelReceipt {model:string;reused:boolean;usage:{prompt_tokens:number|null;completion_tokens:number|null;prompt_cache_hit_tokens:number|null;prompt_cache_miss_tokens:number|null};estimated_cost_usd:number|null;finish_reason?:string|null;price_date?:string|null}
export interface TranscriptionTask {configuration?:TranscriptionOptions;progress?:TaskProgress;resumable?:boolean;task_id: string; status: "pending" | "running" | "succeeded" | "failed" | "cancelled"; result: {word_count?: number; reused?: boolean} | null; error: string | null}
export interface TranscriptionOptions {provider: "mlx" | "whisper"; model: string; language: string}
export interface HighlightClip {instance_id: string; segment_id: string; role: string; text: string; source_text?: string; transcript_text?: string; deleted?: boolean; start_ms: number; end_ms: number}
export const editOutputItem = (project: string, collection: string, output: string, instance: string, changes: {deleted?: boolean; display_text?: string; source_start_ms?:number; source_end_ms?:number}) => projectRequest<HighlightResult>(`/api/projects/${encodeURIComponent(project)}/highlights/${encodeURIComponent(collection)}/outputs/${encodeURIComponent(output)}/items/${encodeURIComponent(instance)}`, {method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(changes)});
...
export interface SplitRange {text:string; start_ms:number; end_ms:number}
export const previewManualSplit = (project:string, collection:string, output:string, item:string, revision:number, lines:string[]) => projectRequest<{ranges:SplitRange[]}>(`/api/projects/${encodeURIComponent(project)}/highlights/${encodeURIComponent(collection)}/outputs/${encodeURIComponent(output)}/items/${encodeURIComponent(item)}/split`, {method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({base_revision:revision,lines,apply:false})});
export const applyManualSplit = (project:string, collection:string, output:string, item:string, revision:number, lines:string[]) => projectRequest<HighlightResult>(`/api/projects/${encodeURIComponent(project)}/highlights/${encodeURIComponent(collection)}/outputs/${encodeURIComponent(output)}/items/${encodeURIComponent(item)}/split`, {method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({base_revision:revision,lines,apply:true})});
