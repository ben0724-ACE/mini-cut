...
export interface HighlightClip {instance_id: string; segment_id: string; role: string; text: string; source_text?: string; deleted?: boolean; start_ms: number; end_ms: number}
export const editOutputItem = (project: string, collection: string, output: string, instance: string, changes: {deleted?: boolean; display_text?: string}) => projectRequest<HighlightResult>(`/api/projects/${encodeURIComponent(project)}/highlights/${encodeURIComponent(collection)}/outputs/${encodeURIComponent(output)}/items/${encodeURIComponent(instance)}`, {method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(changes)});
export interface HighlightOutput {hook_transition_ms?:number;output_id: string; title: string; reason: string; revision: number; duration_ms: number; clips: HighlightClip[]}
export const reorderOutput = (project:string,collection:string,output:string,order:string[],roles:Record<string,string>,hook_transition_ms?:number) => projectRequest<HighlightResult>(`/api/projects/${encodeURIComponent(project)}/highlights/${encodeURIComponent(collection)}/outputs/${encodeURIComponent(output)}/order`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({order,roles,hook_transition_ms})});
export const getOutputVersions = (project:string,collection:string,output:string) => projectRequest<{revision:number;hook_transition_ms?:number;duration_ms:number;clips:HighlightClip[]}[]>(`/api/projects/${encodeURIComponent(project)}/highlights/${encodeURIComponent(collection)}/outputs/${encodeURIComponent(output)}/versions`);
