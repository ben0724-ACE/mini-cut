...
export const startOutputExportBatch=(project:string,collection:string,outputs:{output_id:string;revision:number}[],options:OutputExportOptions,key:string)=>projectRequest<OutputExportTask[]>(`/api/projects/${encodeURIComponent(project)}/tasks/output-export-batch`,{method:"POST",headers:{"Content-Type":"application/json","Idempotency-Key":key},body:JSON.stringify({outputs:outputs.map(output=>({collection_id:collection,...output,...options}))})});
