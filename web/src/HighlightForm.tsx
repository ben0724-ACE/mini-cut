...
export interface HighlightBrief {preset: string; count: number; min_ms: number | null; max_ms: number | null; hook_ms: number | null; instructions: string; max_source_overlap: number; preset_prompt?:string; body_mode?:"continuous"|"compact";translation_language?:"zh"|"en"|null;subtitle_mode?:"bilingual"|"translated"}
export function HighlightForm({ready, busy, onSubmit}: {ready: boolean; busy: boolean; onSubmit: (brief: HighlightBrief) => void}) {
  const [translate,setTranslate]=useState(false);
  const [translationLanguage,setTranslationLanguage]=useState<"zh"|"en">("zh");
  const [subtitleMode,setSubtitleMode]=useState<"bilingual"|"translated">("bilingual");
  ...
  return <form className="transcription-controls" onSubmit={event => {
    ...
    setError(""); onSubmit({translation_language:translate?translationLanguage:null,subtitle_mode:subtitleMode,body_mode:bodyMode,preset, preset_prompt:prompts[preset], instructions, count, min_ms: preset === "clean_speech" ? null : Math.round(min * 1000), max_ms: preset === "clean_speech" ? null : Math.round(max * 1000), hook_ms: hook ? Math.round(hookSeconds * 1000) : null, max_source_overlap: count === 1 ? 1 : overlap / 100});
  }}>
    ...
    <label className="checkbox-field"><input type="checkbox" checked={translate} disabled={busy} onChange={event=>setTranslate(event.target.checked)} />翻译字幕</label>
    {translate&&<><label>翻译为<select aria-label="翻译为" value={translationLanguage} disabled={busy} onChange={e=>setTranslationLanguage(e.target.value as "zh"|"en")}><option value="zh">中文</option><option value="en">英文</option></select></label><label>字幕显示<select aria-label="字幕显示" value={subtitleMode} disabled={busy} onChange={e=>setSubtitleMode(e.target.value as "bilingual"|"translated")}><option value="bilingual">双语（原文＋译文）</option><option value="translated">仅译文</option></select></label><p className="helper-text">候选选定后，调用 DeepSeek 翻译候选字幕，可能计费；不改变原音。译文可在编辑中逐句修改，成片预览及导出使用所选显示方式。</p></>}
