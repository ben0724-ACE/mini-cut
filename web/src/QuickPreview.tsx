...
export function QuickPreview({sourceUrl,title,clips,audition,onDuration}:{sourceUrl:string;title:string;clips:HighlightClip[];audition?:Audition;onDuration:(ms:number)=>void}) {
  ...
  function tick(){const element=video.current;const current=queue.current[index.current];if(!element||!current)return;if(element.currentTime*1000>=current.end){index.current++;const next=queue.current[index.current];if(next){element.currentTime=next.start/1000;}else{element.pause();element.currentTime=current.end/1000;queue.current=[];}}}
