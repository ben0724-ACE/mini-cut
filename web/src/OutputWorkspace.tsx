...
export function OutputWorkspace({project,collection,output,compose,onUpdated}:{project:string;collection:string;output:string;compose?:(panels:Panels)=>ReactNode;onUpdated?:(result:HighlightResult)=>void}) {
  ...
  const [versions,setVersions]=useState<{revision:number;hook_transition_ms?:number;duration_ms:number;clips:HighlightClip[]}[]>([]);
  const [historyError,setHistoryError]=useState("");const [viewVersion,setViewVersion]=useState<number|undefined>();
  async function update(operation:()=>Promise<HighlightResult>){if(lock.current)throw new Error("请等待当前保存完成");lock.current=true;setBusy(true);try{const updated=await operation();setJumpTo(undefined);setResult(updated);onUpdated?.(updated);setViewVersion(undefined);setVersions([]);}finally{lock.current=false;setBusy(false);}}
