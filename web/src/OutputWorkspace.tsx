...
export function OutputWorkspace({project,collection,output,compose,onUpdated}:{project:string;collection:string;output:string;compose?:(panels:Panels)=>ReactNode;onUpdated?:(result:HighlightResult)=>void}) {
  ...
  const preview=<><h1 className="preview-title">{plan.title}</h1><p className="muted">v{viewVersion??plan.revision} · {(plan.duration_ms/1000).toFixed(1)} 秒</p><RenderedPreview key={`${collection}:${output}:${viewVersion??plan.revision}`} project={project} collection={collection} output={output} revision={viewVersion??plan.revision} title={plan.title} jumpTo={jumpTo} sourceUrl={`/api/projects/${encodeURIComponent(project)}/media/source/${encodeURIComponent(result.asset_id)}`} /></>;
